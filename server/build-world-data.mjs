...
//
// ⚠ The build is exported as buildWorldData() and only WRITTEN when this file is
// run directly. tools/check_world_data.mjs imports the same function to detect a
// STALE committed file — if the checker reimplemented the packing instead, the
// two would drift and the check would start passing on a file that is wrong,
...
export async function buildWorldData({ quiet = false } = {}) {
  ...
  if (existsSync(wePath)) {
    try {
      ...
      if (!quiet) console.log('applied world_edits.json map overrides:', we.map ? Object.keys(we.map).length : 0);
    } catch (e) { if (!quiet) console.warn('world_edits.json not applied:', e.message); }
  }
  ...
  return { out, stats: { portals: portals.length, resCount } };
}

// Only write when invoked directly — importing this must have no side effects.
if (import.meta.url === pathToFileURL(process.argv[1] || '').href) {
  const { out, stats } = await buildWorldData();
  writeFileSync(path.join(here, 'world-data.json'), JSON.stringify(out));
  console.log('wrote server/world-data.json —',
    `${stats.portals} portals, ${out.wolfSpawns.length} wolf + ${out.banditSpawns.length} bandit spawns, ${stats.resCount} resource tiles`);
}
